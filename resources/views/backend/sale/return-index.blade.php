...
<div class="roles-permissions">
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-gray-700 uppercase font-bold">All Returns</h2>
    ...
    <table class="min-w-full table-auto border-collapse">
        <thead>
            <tr class="bg-gray-600 text-white uppercase text-sm font-semibold">
                ...
                <th class="px-4 py-3 text-left">Sale Total</th>
                <th class="px-4 py-3 text-left">Refund Total</th>
                <th class="px-4 py-3 text-left">Sale Date</th>
                <th class="px-4 py-3 text-left">Return Date</th>
                <th class="px-4 py-3 text-left">Sale By</th>
                <th class="px-4 py-3 text-left">Return By</th>
        ...
        <tbody>
            ...
            <tr class="border-t">
                <td class="px-4 py-3">{{ $return->bill_no }}</td>
                <td class="px-4 py-3">{{ $return->saleBill->parent->user->name }}</td>
                <td class="px-4 py-3">{{ 'Rs ' . $return->saleBill->total ?? '-' }}</td>
                <td class="px-4 py-3">{{ 'Rs ' . $return->total_refund ?? '-' }}</td>
                <td class="px-4 py-3">{{ $return->saleBill->created_at->format('d M Y') ?? '-' }}</td>
                <td class="px-4 py-3">{{ $return->created_at->format('d M Y') ?? '-' }}</td>
                <td class="px-4 py-3">{{ $return->saleBill->sale_done_by->name }}</td>
                <td class="px-4 py-3">{{ $return->return_by->name }}</td>
                <td class="px-4 py-3 text-right space-x-3">
                    <a href="#"
                        class="text-blue-600 hover:text-blue-800 transition transform hover:scale-125">
                        <i class="fa fa-pencil text-sm"></i>
                    </a>
                    <a href="{{ route('return.show', $return->id) }}"
