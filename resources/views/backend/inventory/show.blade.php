...
<div class="roles-permissions">
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-gray-700 uppercase font-bold">Inventory Invoice</h2>

        <a href="{{ route('inventory.invoice-download', $purchase->id) }}"
            class="bg-gray-700 text-white text-sm uppercase py-2 px-4 flex items-center rounded">
           <i class="fas fa-download mr-2"></i> Download Purchase Invoice
        </a>
    </div>
</div>
<div class="bg-white p-6 rounded-xl shadow-sm mb-8 border border-gray-200 max-w-4xl mx-auto">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-gray-700">
        <div class="bg-gray-50 px-4 py-2.5 rounded-lg flex justify-between items-center">
            ...
            <span class="font-semibold text-gray-800">{{ $purchase->invoice_no }}</span>
        </div>
        <div class="bg-gray-50 px-4 py-2.5 rounded-lg flex justify-between items-center">
            <span class="text-gray-500 font-medium">Vendor Name:</span>
            <span class="font-semibold text-gray-800">{{ $purchase->vendor->vendor_name }}</span>
        </div>

        <div class="bg-gray-50 px-4 py-2.5 rounded-lg flex justify-between items-center">
            <span class="text-gray-500 font-medium">Total Amount:</span>
            <span class="font-semibold text-gray-800">Rs. {{ $purchase->total }}</span>
        </div>

        <div class="bg-gray-50 px-4 py-2.5 rounded-lg flex justify-between items-center">
            <span class="text-gray-500 font-medium">Date:</span>
            <span class="font-semibold text-gray-800">{{ $purchase->created_at }}</span>
        </div>

        <div class="bg-gray-50 px-4 py-2.5 rounded-lg flex justify-between items-center">
            <span class="text-gray-500 font-medium">By:</span>
            <span class="font-semibold text-gray-800">{{ $purchase->inv_added_by->name }}</span>
        </div>
    </div>
</div>
...
<div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200 w-full">
        ...
        <table class="min-w-full border border-gray-300 text-sm text-center">
            <thead class="bg-gray-100">
                <tr>
                    ...
                    <th class="border px-3 py-2">Cost Price</th>
                    <th class="border px-3 py-2">Selling Price</th>
                    <th class="border px-3 py-2">Quantity</th>
                    <th class="border px-3 py-2">Total</th>
                </tr>
            </thead>
            <tbody>
                @foreach($purchase->children as $index => $child)
                <tr class="hover:bg-gray-50">
                    <td class="border px-3 py-2">{{ $child->item->name ?? 'N/A' }}</td>
                    <td class="border px-3 py-2">{{ strtoupper($child->variant) }}</td>
                    <td class="border px-3 py-2">₹ {{  $child->cost_price }}</td>
                    <td class="border px-3 py-2">₹ {{ $child->selling_price }}</td>
    ...
    <div class="mt-4 text-right text-sm text-gray-700">
        <strong>GrandTotal:</strong> Rs. {{ $purchase->total }}
    </div>
</div>
@endsection