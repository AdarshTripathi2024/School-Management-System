...
@section('content')
    ...
    <div class="roles-permissions">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-gray-700 uppercase font-bold">Vendors</h2>
            </div>
            <div class="flex flex-wrap items-center">
                <button class="bg-green-500 text-white text-sm uppercase py-2 px-4 flex items-center rounded"  id="openAddVendorModal">
                    <svg class="w-3 h-3 fill-current" aria-hidden="true" focusable="false" data-prefix="fas" data-icon="plus" class="svg-inline--fa fa-plus fa-w-14" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path fill="currentColor" d="M416 208H272V64c0-17.67-14.33-32-32-32h-32c-17.67 0-32 14.33-32 32v144H32c-17.67 0-32 14.33-32 32v32c0 17.67 14.33 32 32 32h144v144c0 17.67 14.33 32 32 32h32c17.67 0 32-14.33 32-32V304h144c17.67 0 32-14.33 32-32v-32c0-17.67-14.33-32-32-32z"></path></svg>
                    <span class="ml-2 text-xs font-semibold">Add New Vendor</span>
                </a>
            </div>
        </div>
           ...
           <div class="w-1/12 px-4 py-3">#</div>
            ...
            @foreach ($vendors as $vendor)
                <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    ...
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $vendor->vendor_name }}</div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $vendor->mobile }}</div>
                    <div class="w-3/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $vendor->address }}</div>
                    <div class="w-2/12 flex items-center justify-end px-3">
                        <a href="{{ route('vendor.edit',$vendor->id) }}" title="Edit">
                 ...
                 <form action="{{ route('vendor.destroy', $vendor->id) }}" method="POST" onsubmit="return confirm('Are you sure?');" class="inline">
                    @csrf
                    @method('DELETE')
        ...
        <div class="mt-8">
         ...
         @include('backend.modals.delete',['name' => 'vendor'])
    </div>
